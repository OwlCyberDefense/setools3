...
####										  ####
#										     # 	
# The following are primitive functions for loading and retrieving a permmission map,#
...
proc Apol_Perms_Map::load_perm_map_from_mls {mls_file save_file} {
	...
	if {[array exists class_info]} {
		set rt [catch {Apol_Perms_Map::write_perm_map_file "class_info" $save_file} err]
...
########################################################################
# ::save_permission_map --
#	- Primitive procedure for saving a perm map file to disk
#
...
########################################################################
# ::apply_permission_map --
#	- Primitive procedure for updating perm map in memory.
#
proc Apol_Perms_Map::apply_permission_map {} { 
	# create a temp filename with WRONLY access only!
        if { [catch {set tmpfilename [Apol_Perms_Map::create_tmp_file]} err] } {
        	return -code error $err
        }

	set rt [catch {Apol_Perms_Map::save_permission_map $tmpfilename} err]
	if {$rt != 0} {
		return -code error $err	
	}
...
#	   	  created.  
#
proc Apol_Perms_Map::write_perm_map_file {classes pmap_file {mls_file ""}} {	
...
############################################################################################
# The following are load perm map interfaces, which are called from the top-level GUI only #
############################################################################################
...
proc Apol_Perms_Map::load_perm_map_fileDlg {parentDlg} {
	...
	# Check for changes to current 	
	Apol_Perms_Map::checkAndSaveChanges $parentDlg 
	set pmap_file ""
        set types {
		{"All files"		*}
    	}
...
########################################################################
# The following procedures are related to the perm map editor GUI only #
########################################################################
...
proc Apol_Perms_Map::initialize { } {
...
########################################################################
# ::reset_perms_mappings_vars -- 
#	- Resets all TCL variables related to the permission map.
proc Apol_Perms_Map::reset_perms_mappings_vars { } {
	variable mls_base_perms_array
	variable selinux_perms_array
	variable mls_classes_list
	variable undefined_perm_classes
...
# ::close_Dlg --
#	- 
proc Apol_Perms_Map::close_Dlg { {override "0"} } {
	variable edit_flag 
	
	if {$edit_flag && !$override} {
		set ans [tk_messageBox -icon question -type yesno -title "Exit Perm Map Editor?" \
			-parent $Apol_Perms_Map::perm_mappings_Dlg \
			-message "There were unsaved or unapplied changes to the perm map. Choose 'Yes' \
			to exit the perm map editor without applying or saving these changes. Choose \
			'No' to keep the perm map editor open."]
		if {$ans == "no"} {
			return 
		}
	}
	# NOTE: Here we don't want to call Apol_Perms_Map::reset_perms_mappings_vars because we may 
	# need this information to save out to disk if there was an applied but unsaved perm map and
	# the user tries to Exit apol or close the policy.
...
# ::load_class_info --
#	- 
proc Apol_Perms_Map::load_class_info { } {
	variable mls_base_perms_array
	variable selinux_perms_array
	variable mls_classes_list
	variable loaded_pmap
	...
	Apol_Perms_Map::reset_perms_mappings_vars
	...
	for {set j 0} {$j < $num_classes} {incr j} {
		...
		for {set k 0} {$k < $num_perms} {incr k} {
			...
			# Gather a list of classes that have undefined permission mappings
...
proc Apol_Perms_Map::embed_mls_perms_widgets {list_b class selinux_perm} {
	...
	set cb_frame [frame $frame.cb_frame:$selinux_perm -width 10 -bd 0 -bg white]
...
#
proc Apol_Perms_Map::render_perm_mappings {class_name} {
	variable perm_mappings_Dlg
	variable perms_mappings_lb
	variable selinux_perms_array
...
proc Apol_Perms_Map::refresh_perm_mappings { } {
	...
	Apol_Perms_Map::reset_perms_mappings_vars
	# Load object classes and related information. 
	set rt [catch {Apol_Perms_Map::load_class_info} err]
	if {$rt != 0} {
		return -code error $err	
	}
	if {$sel_idx != ""} {
		set class_name [$Apol_Perms_Map::class_listbox get $sel_idx]
		set rt [catch {Apol_Perms_Map::render_perm_mappings $class_name} err]
		if {$rt != 0} {
			return -code error $err	
		}
...
proc Apol_Perms_Map::indicate_undef_perm_classes {class_listbox } {
	...
	$class_listbox configure -listvar Apol_Perms_Map::mls_classes_list
...
proc Apol_Perms_Map::create { } {
	...
	# Create the top-level dialog and subordinate widgets
        ...
        bind permMap_list_Tag <<ListboxSelect>> {Apol_Perms_Map::render_perm_mappings [$Apol_Perms_Map::class_listbox get [$Apol_Perms_Map::class_listbox curselection]]}
	...
	set b_exit [button $botf.b_exit -text "Exit" -width 8 -command {Apol_Perms_Map::close_Dlg 1}]	            
	set b_save   [button $botf.b_save -text "Save" -width 8 -command {Apol_Perms_Map::save_perm_map_Dlg $Apol_Perms_Map::perm_mappings_Dlg}]
	...
	pack $sw_list -fill both -expand yes 
        pack $b_save $b_saveas $b_saveas_Dflt $b_exit -side left -padx 5 -pady 5 -anchor center
...
proc Apol_Perms_Map::display_perm_mappings_Dlg { } {
	...
	Apol_Perms_Map::initialize
	# Load object classes and related information. Then once we have this data, we can create the widgets.
	set rt [catch {Apol_Perms_Map::load_class_info} err]
	if {$rt != 0} {
		tk_messageBox -icon error -type ok -title "Error" -message "$err"
		return -1
	}
	set rt [catch {Apol_Perms_Map::create} err]
	if {$rt != 0} {
		tk_messageBox -icon error -type ok -title "Error" -message "$err"
		return -1
	}
...
################################################################################
# The following procedures are related to checking and saving applied changes 
# to a perm map, which have not been saved out to disk. These are called from 
# the message box which is displayed on the following events:
#	1. When closing the policy from the top-level GUI menu. 
#	2. When exiting the application from the top-level GUI menu.
# 	3. When loading a permission map explicitly from the top-level GUI menu.
################################################################################
...
# ::checkAndSaveChanges --
#	- 
proc Apol_Perms_Map::checkAndSaveChanges {parent} {
	variable edit_flag 
	variable saveChanges_Dialog_ans 
	variable saved_flag 
	variable system_dflt_flg
	
	if {$edit_flag && !$saved_flag} {
		while {1} {
			Apol_Perms_Map::display_saveChanges_Dialog $system_dflt_flg
			switch -exact -- $saveChanges_Dialog_ans {
				"save" {
					# Save perm map out to disk
					set rt [Apol_Perms_Map::save_edited_pmap $parent]
					break
				}
				"saveAs" {
					# Save perm map as...
					set rt [Apol_Perms_Map::save_edited_pmap_as $parent] 
					if {$rt} {
						continue	
					}
					break
				...
				"saveDef" {
					# Save perm map to the user default location
					set rt [Apol_Perms_Map::save_edited_pmap_as_dflt $parent]
					break
				}
				default {
					# No (don't save) or anything else
					set saved_flag 0
					break
				}
			}		
...
#  	-
proc Apol_Perms_Map::display_saveChanges_Dialog {is_sys_dflt_pmap} {	
	variable saveChanges_Dialog
	variable saved_flag
	global tcl_platform
	...
	wm protocol $saveChanges_Dialog WM_DELETE_WINDOW ""
	wm withdraw $saveChanges_Dialog
	wm title $saveChanges_Dialog "Save perm map changes?"
	...
	set inner_f [frame $saveChanges_Dialog.inner_f]
    	...
    	set lbl_save2  [label $inner_f2.lbl_save2 -text "There were changes to the current permission map. Would you like to save these changes?" \
    			-font {Helvetica 11}]
    	if {!$is_sys_dflt_pmap} {
    		set b_save [button $inner_f.b_yes -text "Save" -width 6 -command {set Apol_Perms_Map::saveChanges_Dialog_ans "save"; destroy $Apol_Perms_Map::saveChanges_Dialog} -font {Helvetica 11}]
    	}
	set b_saveAs [button $inner_f.b_no -text "Save As" -width 6 -command {set Apol_Perms_Map::saveChanges_Dialog_ans "saveAs"; destroy $Apol_Perms_Map::saveChanges_Dialog} -font {Helvetica 11}]
	set b_default [button $inner_f.b_discard -text "Save As User Default" -command {set Apol_Perms_Map::saveChanges_Dialog_ans "saveDef"; destroy $Apol_Perms_Map::saveChanges_Dialog} -font {Helvetica 11}]
	set b_no [button $inner_f.b_cancel -text "No" -width 6 -command {set Apol_Perms_Map::saveChanges_Dialog_ans "no"; destroy $Apol_Perms_Map::saveChanges_Dialog} -font {Helvetica 11}]
	...
	if {!$is_sys_dflt_pmap} {
		pack $b_save -side left -anchor center -padx 2 -pady 2
	}
	pack $b_saveAs $b_default $b_no -side left -anchor center -padx 2 -pady 2
	...
	if {$tcl_platform(platform) == "windows"} {
		wm resizable $Apol_Perms_Map::::saveChanges_Dialog 0 0
	} else {
		bind $Apol_Perms_Map::::saveChanges_Dialog <Configure> { wm geometry $Apol_Perms_Map::::saveChanges_Dialog {} }
	}
	
	vwait Apol_Perms_Map::saveChanges_Dialog_ans 
...
# ::save_edited_pmap
#  	- 
proc Apol_Perms_Map::save_edited_pmap {parentDlg} {
	variable loaded_pmap
	variable edit_flag
	variable saved_flag
	...
	if {$loaded_pmap != ""} {
		set rt [catch {Apol_Perms_Map::save_permission_map $loaded_pmap} err]
		if {$rt != 0} {
			tk_messageBox -icon error -type ok -title "Error" \
				-message "Error: $err" -parent $parentDlg	 
			return -1
		}
		set saved_flag 1
	} else {
		tk_messageBox -icon error -type ok -title "Error" \
			-message "No loaded perm map to save." -parent $parentDlg	 
		return -1
	}
	return 0
}
...
# ::save_edited_pmap_as
#  	- 
proc Apol_Perms_Map::save_edited_pmap_as {parentDlg} {
	variable user_default_pmap
	variable edit_flag
	variable saved_flag
	...
	if {$pmap_file != ""} {
		# Check if this is being saved as the users' default perm map.
		if {$pmap_file == $user_default_pmap} {
			set rt [Apol_Perms_Map::save_edited_pmap_as_dflt $parentDlg]
			if {$rt != 0} {
				return -1	
			}
		} else {
...
###################################################################
# ::save_edited_pmap_as_dflt
#  	- 
proc Apol_Perms_Map::save_edited_pmap_as_dflt {parentDlg} {
	variable user_default_pmap
	variable edit_flag
	variable saved_flag
