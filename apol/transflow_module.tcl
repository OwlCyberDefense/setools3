...
# ------------------------------------------------------------------------------
#  Command Apol_Analysis_transflow::verify_options
# ------------------------------------------------------------------------------
proc Apol_Analysis_transflow::verify_options {} {
	variable start_type
        variable end_type
...
## Command Apol_Analysis_transflow::do_analysis is the principal interface command.
...
# ------------------------------------------------------------------------------
#  Command Apol_Analysis_transflow::do_analysis
# ------------------------------------------------------------------------------
proc Apol_Analysis_transflow::do_analysis { results_frame } {
	variable start_type
        variable end_type
        variable endtype_sel
	variable transflow_tree
	...
	# if a permap is not loaded then load the default permap
        ...
        set rt [catch {Apol_Analysis_transflow::load_default_perm_map} err]
	if {$rt != 0} {
		return -code error $err
	}
	...
	if {![array exists f_opts] || [array names f_opts "$advanced_filter_Dlg,name"] == ""} {
		Apol_Analysis_transflow::advanced_filters_create_object $advanced_filter_Dlg
	}
	...
	foreach class $f_opts($advanced_filter_Dlg,class_list) {
		...
		set idx [string first $Apol_Analysis_transflow::excluded_tag $class]
	...
	Apol_Analysis_transflow::display_progressDlg
	...
	if {$rt != 0} {
		Apol_Analysis_transflow::destroy_progressDlg
	        tk_messageBox -icon error -type ok -title "Error" -message "$err"
		return -code error $err
	}
	set query_args [list \
	...
	set transflow_tree [Apol_Analysis_transflow::create_resultsDisplay $results_frame]
	set rt [catch {Apol_Analysis_transflow::create_result_tree_structure $transflow_tree $results $query_args} err]
	if {$rt != 0} {
		Apol_Analysis_transflow::destroy_progressDlg
		tk_messageBox -icon error -type ok -title "Error" -message "$err"
		return -code error $err
	...
	Apol_Analysis_transflow::destroy_progressDlg
	set Apol_Analysis_transflow::progress_indicator -1
	return 0
}

proc Apol_Analysis_transflow::treeSelect {transflow_tree transflow_info_text node} {
	# Set the tree selection to the current node.
	$transflow_tree selection set $node

        if {$node == [$transflow_tree nodes root]} {
		Apol_Analysis_transflow::display_root_type_info $node $transflow_info_text $transflow_tree
	        Apol_Analysis_transflow::formatInfoText $transflow_info_text
	} else {
		Apol_Analysis_transflow::insert_transitive_flows_header $transflow_info_text $transflow_tree $node
		Apol_Analysis_transflow::render_information_flows $transflow_info_text $transflow_tree $node
		Apol_Analysis_transflow::formatInfoText $transflow_info_text
	}
	ApolTop::makeTextBoxReadOnly $transflow_info_text
	return 0
}
...
#
proc Apol_Analysis_transflow::find_more_flows_generate_virtual_events {} {
	variable find_flows_results_Dlg

	bind $find_flows_results_Dlg <<FindMoreFlowsStarted>> {
		set elapsed_time [Apol_Analysis_transflow::convert_seconds \
			[expr [clock seconds] - $Apol_Analysis_transflow::start_time]]
		$Apol_Analysis_transflow::time_exp_lbl configure -text $elapsed_time
	}
...
#
proc Apol_Analysis_transflow::display_root_type_info { source_type transflow_info_text transflow_tree } {
	...
	$transflow_info_text insert end "Transitive Information Flow Analysis: Starting type: "
	set endIdx [$transflow_info_text index insert]
	$transflow_info_text tag add $Apol_Analysis_transflow::title_tag $startIdx $endIdx
	set startIdx $endIdx
	$transflow_info_text insert end $source_type
	set endIdx [$transflow_info_text index insert]
	$transflow_info_text tag add $Apol_Analysis_transflow::title_type_tag $startIdx $endIdx
	set startIdx $endIdx
	# now add the standard text
	$transflow_info_text configure -wrap word
	set start_idx [$transflow_info_text index insert]
	$transflow_info_text insert end $Apol_Analysis_transflow::root_text
	$transflow_info_text tag add ROOT_TEXT $start_idx end
	$transflow_info_text tag configure ROOT_TEXT -font $ApolTop::text_font
...
# ::formatInfoText
#
proc Apol_Analysis_transflow::formatInfoText { tb } {
	$tb tag configure $Apol_Analysis_transflow::title_tag -font {Helvetica 14 bold}
	$tb tag configure $Apol_Analysis_transflow::title_type_tag -foreground blue -font {Helvetica 14 bold}
	$tb tag configure $Apol_Analysis_transflow::subtitle_tag -font {Helvetica 11 bold}
	$tb tag configure $Apol_Analysis_transflow::rules_tag -font $ApolTop::text_font
	$tb tag configure $Apol_Analysis_transflow::counters_tag -foreground blue -font {Helvetica 11 bold}
	$tb tag configure $Apol_Analysis_transflow::types_tag -font $ApolTop::text_font
	$tb tag configure $Apol_Analysis_transflow::find_flows_tag -font {Helvetica 14 bold} -foreground blue -underline 1
	$tb tag configure $Apol_Analysis_transflow::disabled_rule_tag -foreground red

	$tb tag bind $Apol_Analysis_transflow::find_flows_tag <Button-1> "Apol_Analysis_transflow::display_find_more_flows_Dlg"
	$tb tag bind $Apol_Analysis_transflow::find_flows_tag <Enter> { set Apol_Analysis_transflow::orig_cursor [%W cget -cursor]; %W configure -cursor hand2 }
	$tb tag bind $Apol_Analysis_transflow::find_flows_tag <Leave> { %W configure -cursor $Apol_Analysis_transflow::orig_cursor }

	# Configure hyperlinking to policy.conf file
	Apol_PolicyConf::configure_HyperLinks $tb
}

proc Apol_Analysis_transflow::insert_src_type_node { transflow_tree query_args} {
        variable start_type

	$transflow_tree insert end root $start_type \
		-text $start_type \
		-open 1	\
	-drawcross auto \
		-data $query_args

        return [$transflow_tree nodes root]
}

proc Apol_Analysis_transflow::create_target_type_nodes { parent transflow_tree results_list } {
        if { [file tail [$transflow_tree parent $parent]] == [file tail $parent] } {
		return
	}

	if { [$transflow_tree nodes $parent] == "" } {
		# Get # of target types (if none, then just draw the tree without child nodes)
		# We skip index 0 b/c it is the starting type, which we already have.
		...
		for { set x 0 } {$x < $num_target_types} { incr x } {
			...
			set nextIdx [Apol_Analysis_transflow::parseList_get_index_next_node $curentIdx $results_list]
			if {$nextIdx == -1} {
				return -code error "Error parsing results. See stdout for more information."
			}

			set target_node "${parent}/${target_name}/"
			$transflow_tree insert end $parent $target_node \
				-text $target_name \
				-open 0	\
			-drawcross allways \
		...
		set nodes [lsort [$transflow_tree nodes $parent]]
		$transflow_tree reorder $parent $nodes
	        $transflow_tree configure -redraw 1
	}
        return 0
}

proc Apol_Analysis_transflow::parseList_get_index_next_node { currentIdx results_list } {
	# Increment to # paths
	incr currentIdx
...
proc Apol_Analysis_transflow::create_result_tree_structure { transflow_tree results_list query_args} {
        set home_node [Apol_Analysis_transflow::insert_src_type_node $transflow_tree $query_args]
	set rt [catch {Apol_Analysis_transflow::create_target_type_nodes $home_node $transflow_tree $results_list} err]
	if {$rt != 0} {
		return -code error $err
	}

	Apol_Analysis_transflow::treeSelect \
		$Apol_Analysis_transflow::transflow_tree $Apol_Analysis_transflow::transflow_info_text $home_node
        return 0
}

# ------------------------------------------------------------------------------
#  Command Apol_Analysis_transflow::do_child_analysis
# ------------------------------------------------------------------------------
proc Apol_Analysis_transflow::do_child_analysis { transflow_tree selected_node } {
	ApolTop::setBusyCursor
	Apol_Analysis_transflow::display_progressDlg
	if { [$transflow_tree nodes $selected_node] == "" } {
		# The last query arguments were stored in the data for the root node
		set query_args [$transflow_tree itemcget [$transflow_tree nodes root] -data]
		...
		Apol_Analysis_transflow::create_target_type_nodes $selected_node $transflow_tree $results
	}
	Apol_Analysis_transflow::destroy_progressDlg
...
proc Apol_Analysis_transflow::create_resultsDisplay { results_frame } {
        variable transflow_tree
        variable transflow_info_text
	...
	# tree window
	set transflow_tree  [Tree [$sw_tree getframe].transflow_tree \
	           -relief flat -borderwidth 0 -highlightthickness 0 \
		   -redraw 0 -bg white -showlines 1 -padx 0 \
		   -opencmd  {Apol_Analysis_transflow::do_child_analysis $Apol_Analysis_transflow::transflow_tree}]
	$sw_tree setwidget $transflow_tree

	# info window
	set transflow_info_text [text [$sw_info getframe].transflow_info_text -wrap none -bg white -font $ApolTop::text_font]
	$sw_info setwidget $transflow_info_text
	bind $transflow_info_text <Enter> {focus %W}
	...
	$transflow_tree bindText  <ButtonPress-1> {Apol_Analysis_transflow::treeSelect \
		$Apol_Analysis_transflow::transflow_tree $Apol_Analysis_transflow::transflow_info_text}

	$transflow_tree bindText  <Double-ButtonPress-1> {Apol_Analysis_transflow::treeSelect \
		$Apol_Analysis_transflow::transflow_tree $Apol_Analysis_transflow::transflow_info_text}

	return $transflow_tree
}


# ------------------------------------------------------------------------------
#  Command Apol_Analysis_transflow::load_default_perm_map
# ------------------------------------------------------------------------------
proc Apol_Analysis_transflow::load_default_perm_map {} {
	# if a permap is not loaded then load the default permap
...
# ------------------------------------------------------------------------------
#  Command Apol_Analysis_transflow::advanced_filters_refresh_dialog
# ------------------------------------------------------------------------------
proc Apol_Analysis_transflow::advanced_filters_refresh_dialog {path_name} {
	if {[array exists f_opts] && \
	    [array names f_opts "$path_name,name"] != ""} {
		Apol_Analysis_transflow::advanced_filters_destroy_object $path_name
		Apol_Analysis_transflow::advanced_filters_create_object $path_name
		Apol_Analysis_transflow::advanced_filters_update_dialog $path_name
	}
...
# ------------------------------------------------------------------------------
#  Command Apol_Analysis_transflow::advanced_filters_update_dialog
# ------------------------------------------------------------------------------
proc Apol_Analysis_transflow::advanced_filters_update_dialog {path_name} {
	variable f_opts
	...
	if {[array exists f_opts] && \
		...
		set rt [catch {Apol_Analysis_transflow::advanced_filters_set_widgets_to_default_state \
			$path_name} err]
		if {$rt != 0} {
			tk_messageBox -icon error -type ok -title "Error" -message "$err"
			return -1
		...
		if {$f_opts($path_name,class_selected_idx) != "-1"} {
			...
			Apol_Analysis_transflow::advanced_filters_display_permissions $path_name
		}
	}
...
# ------------------------------------------------------------------------------
#  Command Apol_Analysis_transflow::advanced_filters_include_types
...
# ------------------------------------------------------------------------------
proc Apol_Analysis_transflow::advanced_filters_include_types {remove_list_1 \
							    add_list_1 \
							    remove_lbox \
							    add_lbox \
...
# ------------------------------------------------------------------------------
#  Command Apol_Analysis_transflow::advanced_filters_exclude_types
#	- remove_list - the list displayed inside the listbox from which the
#			type is being removed.
#	- add_list - the list displayed inside the listbox to which the type
#		     being added.
...
#	- add_lbox - listbox widget to which the type is being added.
# ------------------------------------------------------------------------------
proc Apol_Analysis_transflow::advanced_filters_exclude_types {remove_list_1 \
							    add_list_1 \
							    remove_lbox \
							    add_lbox \
							    master_incl_types_list_1 \
...
# ------------------------------------------------------------------------------
#  Command Apol_Analysis_transflow::advanced_filters_configure_adv_combo_state
# ------------------------------------------------------------------------------
proc Apol_Analysis_transflow::advanced_filters_configure_adv_combo_state {cb_selected_1 \
									combo_box \
									lbox \
									which_list \
									path_name} {
	...
	if {$cb_selected} {
		...
		if {$which_list == "incl"} {
			Apol_Analysis_transflow::advanced_filters_filter_types_using_attrib \
				Apol_Analysis_transflow::f_opts($path_name,incl_attrib_combo_value) \
				$lbox \
				Apol_Analysis_transflow::f_opts($path_name,master_incl_types_list)
		} else {
			Apol_Analysis_transflow::advanced_filters_filter_types_using_attrib \
				Apol_Analysis_transflow::f_opts($path_name,excl_attrib_combo_value) \
				$lbox \
				Apol_Analysis_transflow::f_opts($path_name,master_excl_types_list)
		}
	} else {
		...
		if {$which_list == "incl"} {
			...
			tk_messageBox -icon error -type ok -title "Error" \
				...
				Apol_Analysis_transflow::advanced_filters_configure_adv_combo_state. \
...
# ------------------------------------------------------------------------------
#  Command Apol_Analysis_transflow::advanced_filters_filter_types_using_attrib
#	- attribute - the specified attribute
#	- lbox - the listbox in which to perform the selection
# ------------------------------------------------------------------------------
proc Apol_Analysis_transflow::advanced_filters_filter_types_using_attrib {attribute_1 lbox non_filtered_types_1} {
...
# ------------------------------------------------------------------------------
#  Command Apol_Analysis_transflow::advanced_filters_include_exclude_permissions
...
# ------------------------------------------------------------------------------
proc Apol_Analysis_transflow::advanced_filters_include_exclude_permissions {which path_name} {
	variable f_opts

	if {[ApolTop::is_policy_open]} {
		if {[string equal $which "include"] == 0 && [string equal $which "exclude"] == 0} {
			puts "Tcl error: wrong 'which' argument sent to Apol_Analysis_transflow::advanced_filters_include_exclude_permissions. Must be either 'include' or 'exclude'."
			return -1
		}
		...
		foreach object_class_idx $objs {
			...
			set idx [string first $Apol_Analysis_transflow::excluded_tag $object_class]
			if {$idx != -1} {
			...
			if {$object_class_idx != ""} {
				...
				if {[string equal $which "exclude"]} {
					...
					set [$f_opts($path_name,class_listbox) cget -listvar] \
						...
						"$object_class$Apol_Analysis_transflow::excluded_tag"]
				} else {
					$f_opts($path_name,class_listbox) itemconfigure $object_class_idx \
...
# ------------------------------------------------------------------------------
#  Command Apol_Analysis_transflow::advanced_filters_change_obj_state_on_perm_select
#`	-  This proc also searches a class string for the sequence " (Excluded)"
#	   in order to process the class name only.
# ------------------------------------------------------------------------------
proc Apol_Analysis_transflow::advanced_filters_change_obj_state_on_perm_select {path_name} {
	variable f_opts
	...
	if {$f_opts($path_name,class_selected_idx) != "-1"} {
		set class_sel [$f_opts($path_name,class_listbox) get \
			$f_opts($path_name,class_selected_idx)]
		set idx [string first $Apol_Analysis_transflow::excluded_tag $class_sel]
		if {$idx != -1} {
		...
		if {$class_elements != ""} {
			...
			if {$num_excluded == $num_perms_for_class} {
				...
				set [$f_opts($path_name,class_listbox) cget -listvar] \
					...
					$f_opts($path_name,class_selected_idx) \
					"$class_sel$Apol_Analysis_transflow::excluded_tag"]
			} else {
				$f_opts($path_name,class_listbox) itemconfigure \
					$f_opts($path_name,class_selected_idx) \
					-foreground $f_opts($path_name,select_fg_orig)
...
# ------------------------------------------------------------------------------
# Command Apol_Analysis_transflow::advanced_filters_embed_perm_buttons
#	- Embeds include/exclude radiobuttons in the permissions textbox next to
#	  each permission label.
# ------------------------------------------------------------------------------
proc Apol_Analysis_transflow::advanced_filters_embed_perm_buttons {list_b class perm path_name} {
	variable f_opts
	...
	set cb_include [radiobutton $cb_frame.cb_include:$class:$perm -bg white \
		...
		-variable Apol_Analysis_transflow::f_opts($path_name,perm_status_array,$class,$perm) \
		-command "Apol_Analysis_transflow::advanced_filters_change_obj_state_on_perm_select \
			$path_name"]
...
# ------------------------------------------------------------------------------
# Command Apol_Analysis_transflow::advanced_filters_clear_perms_text
# ------------------------------------------------------------------------------
proc Apol_Analysis_transflow::advanced_filters_clear_perms_text {path_name} {
	variable f_opts
...
proc Apol_Analysis_transflow::render_permissions {path_name} {
	variable f_opts
	...
	Apol_Analysis_transflow::advanced_filters_clear_perms_text $path_name
	update
	# Make sure to strip out just the class name, as this may be an excluded class.
	set idx [string first $Apol_Analysis_transflow::excluded_tag $class_name]
	if {$idx != -1} {
	...
	foreach perm $perms_list {
		...
		$f_opts($path_name,perms_box) window create end -window \
			[Apol_Analysis_transflow::advanced_filters_embed_perm_buttons \
...
# ------------------------------------------------------------------------------
# Command Apol_Analysis_transflow::advanced_filters_display_permissions
...
#	  listbox.
# ------------------------------------------------------------------------------
proc Apol_Analysis_transflow::advanced_filters_display_permissions {path_name} {
	variable f_opts
	...
	bind ${bind_tag_id}_transflow_object_list_Tag <<ListboxSelect>> ""
	set f_opts($path_name,class_selected_idx) [$f_opts($path_name,class_listbox) curselection]
	Apol_Analysis_transflow::render_permissions $path_name
	update idletasks
	bind ${bind_tag_id}_transflow_object_list_Tag <<ListboxSelect>> \
		"Apol_Analysis_transflow::advanced_filters_display_permissions $path_name"
}
...
# ------------------------------------------------------------------------------
#  Command Apol_Analysis_transflow::advanced_filters_initialize_objs_and_perm_filters
# ------------------------------------------------------------------------------
proc Apol_Analysis_transflow::advanced_filters_initialize_objs_and_perm_filters {path_name} {
	variable f_opts
...
# ------------------------------------------------------------------------------
#  Command Apol_Analysis_transflow::advanced_filters_initialize_vars
# ------------------------------------------------------------------------------
proc Apol_Analysis_transflow::advanced_filters_initialize_vars {path_name} {
	variable f_opts
	...
	if {$f_opts($path_name,filter_vars_init) == 0} {
		...
		Apol_Analysis_transflow::advanced_filters_initialize_objs_and_perm_filters $path_name
...
# ------------------------------------------------------------------------------
#  Command Apol_Analysis_transflow::advanced_filters_set_widgets_to_default_state
# ------------------------------------------------------------------------------
proc Apol_Analysis_transflow::advanced_filters_set_widgets_to_default_state {path_name} {
	variable f_opts
	...
	# Make sure to strip out just the class name, as this may be an excluded class.
		...
		if {$num_excluded == [llength $class_perms]} {
			set [$f_opts($path_name,class_listbox) cget -listvar] \
				...
				$class_lbox_idx "$class$Apol_Analysis_transflow::excluded_tag"]
	...
	Apol_Analysis_transflow::advanced_filters_configure_adv_combo_state \
		Apol_Analysis_transflow::f_opts($path_name,incl_attrib_cb_sel) \
	...
	Apol_Analysis_transflow::advanced_filters_configure_adv_combo_state \
		Apol_Analysis_transflow::f_opts($path_name,excl_attrib_cb_sel) \
	...
	Apol_Analysis_transflow::advanced_filters_change_spinbox_state \
		$path_name
...
# ------------------------------------------------------------------------------
#  Command Apol_Analysis_transflow::advanced_filters_copy_object
# ------------------------------------------------------------------------------
proc Apol_Analysis_transflow::advanced_filters_copy_object {path_name new_object} {
	variable f_opts
...
# ------------------------------------------------------------------------------
#  Command Apol_Analysis_transflow::advanced_filters_change_spinbox_state
# ------------------------------------------------------------------------------
proc Apol_Analysis_transflow::advanced_filters_change_spinbox_state {path_name} {
	variable f_opts
...
# ------------------------------------------------------------------------------
#  Command Apol_Analysis_transflow::advanced_filters_change_threshhold_value
# ------------------------------------------------------------------------------
proc Apol_Analysis_transflow::advanced_filters_change_threshhold_value {path_name} {
	variable f_opts
...
# ------------------------------------------------------------------------------
#  Command Apol_Analysis_transflow::advanced_filters_create_dialog
# ------------------------------------------------------------------------------
proc Apol_Analysis_transflow::advanced_filters_create_dialog {path_name title_txt} {
	variable f_opts
	...
	-listvar Apol_Analysis_transflow::f_opts($path_name,class_list) \
	-exportselection 0]
	...
	set f_opts($path_name,spinbox_threshhold) [SpinBox $threshhold_frame.spinbox_threshhold \
		...
		-modifycmd "Apol_Analysis_transflow::advanced_filters_change_threshhold_value $path_name"]
	set cbutton_threshhold [checkbutton $threshhold_frame.cbutton_threshhold \
		...
		-variable Apol_Analysis_transflow::f_opts($path_name,threshhold_cb_value) \
	...
	bindtags $f_opts($path_name,class_listbox) \
		...
		${bind_tag_id}_transflow_object_list_Tag]
        bind ${bind_tag_id}_transflow_object_list_Tag \
	<<ListboxSelect>> "Apol_Analysis_transflow::advanced_filters_display_permissions $path_name"
	...
	set f_opts($path_name,lbox_incl) [listbox [$sw_incl getframe].lbox_incl \
		-height 6 \
		-highlightthickness 0 \
		-listvar Apol_Analysis_transflow::f_opts($path_name,filtered_incl_types) \
		-selectmode extended -bg white -exportselection 0]
	set f_opts($path_name,lbox_excl) [listbox [$sw_excl getframe].lbox_excl \
		-height 6 \
		-highlightthickness 0 \
		-listvar Apol_Analysis_transflow::f_opts($path_name,filtered_excl_types) \
		-selectmode extended -bg white -exportselection 0]
	...
	bindtags $f_opts($path_name,lbox_incl) \
		[linsert [bindtags $Apol_Analysis_transflow::f_opts($path_name,lbox_incl)] 3 \
		${bind_tag_id}_lbox_incl_Tag]
	bindtags $f_opts($path_name,lbox_excl) \
		[linsert [bindtags $Apol_Analysis_transflow::f_opts($path_name,lbox_excl)] 3 \
	...
	bind ${bind_tag_id}_lbox_incl_Tag <KeyPress> "ApolTop::tklistbox_select_on_key_callback \
			$Apol_Analysis_transflow::f_opts($path_name,lbox_incl) \
			Apol_Analysis_transflow::f_opts($path_name,filtered_incl_types) \
			%K"
	bind ${bind_tag_id}_lbox_excl_Tag <KeyPress> "ApolTop::tklistbox_select_on_key_callback \
			$Apol_Analysis_transflow::f_opts($path_name,lbox_excl) \
			Apol_Analysis_transflow::f_opts($path_name,filtered_excl_types) \
			%K"
	...
	set b_incl_all_sel [Button $buttons_incl_f.b_incl_all_sel -text "Select All" \
		-command "Apol_Analysis_transflow::select_all_lbox_items \
			$Apol_Analysis_transflow::f_opts($path_name,lbox_incl)"]
	set b_incl_all_clear [Button $buttons_incl_f.b_incl_all_clear -text "Unselect" \
		-command "Apol_Analysis_transflow::clear_all_lbox_items \
			$Apol_Analysis_transflow::f_opts($path_name,lbox_incl)"]
	...
	set f_opts($path_name,combo_incl) [ComboBox $b_incl_f.combo_incl \
		...
		-textvariable Apol_Analysis_transflow::f_opts($path_name,incl_attrib_combo_value) \
		-entrybg $ApolTop::default_bg_color \
		-modifycmd "Apol_Analysis_transflow::advanced_filters_filter_types_using_attrib \
				Apol_Analysis_transflow::f_opts($path_name,incl_attrib_combo_value) \
				$Apol_Analysis_transflow::f_opts($path_name,lbox_incl) \
				Apol_Analysis_transflow::f_opts($path_name,master_incl_types_list)"]
	...
	set f_opts($path_name,combo_excl) [ComboBox [$exclude_f getframe].combo_excl \
		-editable 0 -autopost 1 \
		-textvariable Apol_Analysis_transflow::f_opts($path_name,excl_attrib_combo_value) \
		-entrybg $ApolTop::default_bg_color \
		-modifycmd "Apol_Analysis_transflow::advanced_filters_filter_types_using_attrib \
				Apol_Analysis_transflow::f_opts($path_name,excl_attrib_combo_value) \
				$Apol_Analysis_transflow::f_opts($path_name,lbox_excl) \
	...
	set cb_incl_attrib [checkbutton $b_incl_f.cb_incl_attrib \
		...
		-variable Apol_Analysis_transflow::f_opts($path_name,incl_attrib_cb_sel) \
		-offvalue 0 -onvalue 1 \
		-command "Apol_Analysis_transflow::advanced_filters_configure_adv_combo_state \
			Apol_Analysis_transflow::f_opts($path_name,incl_attrib_cb_sel) \
			$Apol_Analysis_transflow::f_opts($path_name,combo_incl) \
			$Apol_Analysis_transflow::f_opts($path_name,lbox_incl) \
			incl \
			$path_name"]
	set cb_excl_attrib [checkbutton [$exclude_f getframe].cb_excl_attrib \
		-text "Filter excluded type(s) by attribute:" \
		-variable Apol_Analysis_transflow::f_opts($path_name,excl_attrib_cb_sel) \
		-offvalue 0 -onvalue 1 \
		-command "Apol_Analysis_transflow::advanced_filters_configure_adv_combo_state \
			Apol_Analysis_transflow::f_opts($path_name,excl_attrib_cb_sel) \
			$Apol_Analysis_transflow::f_opts($path_name,combo_excl) \
			$Apol_Analysis_transflow::f_opts($path_name,lbox_excl) \
			excl \
			$path_name"]
	...
	set close_bttn [Button $close_frame.close_bttn -text "Close" -width 8 \
		-command "Apol_Analysis_transflow::advanced_filters_destroy_dialog $path_name"]
	...
	Apol_Analysis_transflow::advanced_filters_set_widgets_to_default_state $path_name
	wm protocol $f_opts($path_name,name) WM_DELETE_WINDOW \
		"Apol_Analysis_transflow::advanced_filters_destroy_dialog $path_name"
