...
static int policy_query_find_obj_class(policy_query_obj_options_t *obj_options, int num_obj_options, int obj_class)
{
	int i;
 ...
 * the policy_query_obj_options_t on success or -1 on failure. Checks to
 * prevent the addition of duplicate or contradictory object classes.
 */
int policy_query_add_obj_class(policy_query_obj_options_t **obj_options, int *num_obj_options, int obj_class)
{
	int obj_idx, cur;
	...
	obj_idx = policy_query_find_obj_class(*obj_options, *num_obj_options, obj_class);
	if (obj_idx != -1) {
	...
	cur = *num_obj_options;
	(*num_obj_options)++;
	*obj_options = (policy_query_obj_options_t*)realloc(*obj_options,
						      sizeof(policy_query_obj_options_t)
						      * (*num_obj_options));
	if (!(*obj_options)) {
		fprintf(stderr, "Memory error!\n");
		return -1;
	}
	memset(&(*obj_options)[cur], 0, sizeof(policy_query_obj_options_t));
 ...
 * prevent the addition of duplicate or contradictory object classes.
 */
int policy_query_add_obj_class_perm(policy_query_obj_options_t **obj_options, int *num_obj_options, int obj_class, int perm)
{
	int cur;
	bool_t add = FALSE;
	...
	/* find an existing entry for the object class */
	cur = policy_query_find_obj_class(*obj_options, *num_obj_options, obj_class);
