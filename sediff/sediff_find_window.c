...
static void sediff_scroll_and_highlight_iters(GtkTextView * view, GtkTextBuffer * txt, GtkTextIter * start, GtkTextIter * end)
{
...
static void sediff_find_search_buffer(gpointer user_data)
{
	...
	if (gtk_toggle_button_get_active(fwd)) {
		if (gtk_text_iter_forward_search(&end, gtk_entry_get_text(entry), GTK_TEXT_SEARCH_VISIBLE_ONLY, &start, &end, NULL)) {
			sediff_scroll_and_highlight_iters(view, gui_txt, &start, &end);
...
static int sediff_find_window_init(sediff_find_window_t * find_window)
{
	...
	g_signal_connect(G_OBJECT(button), "clicked", G_CALLBACK(sediff_find_close_button_clicked), find_window);
	button = GTK_BUTTON(glade_xml_get_widget(find_window->xml, "sediff_find_button"));
	g_signal_connect(G_OBJECT(button), "clicked", G_CALLBACK(sediff_find_button_clicked), find_window);

	/* connect the text entry callback events */
	entry = GTK_ENTRY(glade_xml_get_widget(find_window->xml, "sediff_find_text_entry"));
	g_signal_connect(G_OBJECT(entry), "activate", G_CALLBACK(on_sediff_find_entry_activated), find_window);
