...
static gboolean widget_event(GtkWidget * widget, GdkEventMotion * event, gpointer user_data)
{
	...
	if (event->type == GDK_BUTTON_PRESS) {
		...
		gtk_tree_view_get_path_at_pos(treeview, ex, ey, &path, &column, &x, &y);
...
static GtkTreeModel *sediff_create_and_fill_model(poldiff_t * diff)
{
	...
	gtk_tree_store_set(treestore, &topiter,
			   SEDIFF_LABEL_COLUMN, "Summary", SEDIFF_DIFFBIT_COLUMN, 0, SEDIFF_FORM_COLUMN, POLDIFF_FORM_NONE, -1);
	...
	for (i = 0; sediff_items[i].label != NULL; i++) {
		...
		gtk_tree_store_append(treestore, &topiter, NULL);
		g_string_printf(s, "%s %zd", sediff_items[i].label, stats[0] + stats[1] + stats[2] + stats[3] + stats[4]);
		gtk_tree_store_set(treestore, &topiter,
				   SEDIFF_LABEL_COLUMN, s->str,
				   SEDIFF_DIFFBIT_COLUMN, sediff_items[i].bit_pos, SEDIFF_FORM_COLUMN, POLDIFF_FORM_NONE, -1);
		...
		gtk_tree_store_set(treestore, &childiter,
				   SEDIFF_LABEL_COLUMN, s->str,
				   SEDIFF_DIFFBIT_COLUMN, sediff_items[i].bit_pos, SEDIFF_FORM_COLUMN, POLDIFF_FORM_ADDED, -1);
		...
		if (sediff_items[i].has_add_type) {
			...
			gtk_tree_store_set(treestore, &childiter,
					   ...
					   SEDIFF_FORM_COLUMN, POLDIFF_FORM_ADD_TYPE, -1);
		}
		...
		gtk_tree_store_set(treestore, &childiter,
				   SEDIFF_LABEL_COLUMN, s->str,
				   SEDIFF_DIFFBIT_COLUMN, sediff_items[i].bit_pos, SEDIFF_FORM_COLUMN, POLDIFF_FORM_REMOVED, -1);
		...
		if (sediff_items[i].bit_pos != POLDIFF_DIFF_TYPES ||
			...
			gtk_tree_store_set(treestore, &childiter,
					   SEDIFF_LABEL_COLUMN, s->str,
					   SEDIFF_DIFFBIT_COLUMN, sediff_items[i].bit_pos,
					   SEDIFF_FORM_COLUMN, POLDIFF_FORM_MODIFIED, -1);
		}
	}
...
int sediff_get_current_treeview_selected_row(GtkTreeView * tree_view, uint32_t * diffbit, poldiff_form_e * form)
{
	...
	if (gtk_tree_selection_get_selected(sel, &tree_model, &iter)) {
		gtk_tree_model_get(tree_model, &iter, SEDIFF_DIFFBIT_COLUMN, diffbit, SEDIFF_FORM_COLUMN, form, -1);
		return 1;
	} else {
		return 0;
	}
