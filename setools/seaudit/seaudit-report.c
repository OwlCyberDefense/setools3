	...
	if (report_info->configPath == NULL) {
		...
		if (report_info->configPath == NULL) {
			...
			return -1;
	     	...
	     	snprintf(report_info->configPath, len, "%s/%s", configFile_dir, CONFIG_FILE);
	     	free(configFile_dir);
	...
	if (report_info->stylesheet_file == NULL) {
		...
		if (report_info->stylesheet_file == NULL) {
			...
			return -1;
	     	...
	     	snprintf(report_info->stylesheet_file, len, "%s/%s", dir, STYLESHEET_FILE);
	     	free(dir);
	     	assert(report_info->stylesheet_file != NULL);
	}
...
static int seaudit_report_print_custom_section(seaudit_report_info_t report_info,
	...
	if (seaudit_report_is_valid_node_name(name)) {
		if (strcmp(name, "seaudit-report") == 0 && xmlTextReaderNodeType(reader) == 1) {
			...
			if (rt != 0)
				goto err;
			/* NOTE: If a title wasn't provided, we still continue. */
			rt = seaudit_report_print_custom_section(report_info, 
										  reader, 
									     	  title_attr, 
									     	  outfile);
			if (rt != 0)
...
static void seaudit_report_parse_command_line_args(int argc, char **argv, seaudit_report_info_t *report_info) {
	...
	if (report_info->stylesheet_file == NULL && report_info->html) {
		/* Import stylesheet in /usr/share/setools directory by default. If the default  
