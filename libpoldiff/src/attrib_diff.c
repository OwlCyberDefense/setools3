...
static int attrib_name_comp(const void *x, const void *y, void *arg) {
	...
	char *name1, *name2;
	if (qpol_type_get_name(p->qh, p->p, r1, &name1) < 0 ||
	    qpol_type_get_name(p->qh, p->p, r2, &name2) < 0) {
		return 0;
	}
...
int attrib_comp(const void *x, const void *y, poldiff_t *diff)
{
	...
	qpol_type_t *r2 = (qpol_type_t *) y;
	char *name1, *name2;
	if (qpol_type_get_name(diff->orig_pol->qh, diff->orig_pol->p, r1, &name1) < 0 ||
	    qpol_type_get_name(diff->mod_pol->qh, diff->mod_pol->p, r2, &name2) < 0) {
		return 0;
	}
	return strcmp(name1, name2);
}
...
int attrib_new_diff(poldiff_t *diff, poldiff_form_e form, const void *item)
{
	...
	if ((form == POLDIFF_FORM_ADDED &&
	     qpol_type_get_name(diff->mod_pol->qh, diff->mod_pol->p, r, &name) < 0) ||
	    ((form == POLDIFF_FORM_REMOVED || form == POLDIFF_FORM_MODIFIED) &&
	     qpol_type_get_name(diff->orig_pol->qh, diff->orig_pol->p, r, &name) < 0)) {
		return -1;
	}
...
static apol_vector_t *attrib_get_types(poldiff_t *diff, qpol_type_t *attrib, int which)
{
	...
	if (which == POLDIFF_POLICY_ORIG) {
		if (qpol_type_get_type_iter(diff->orig_pol->qh, diff->orig_pol->p, attrib, &iter) < 0) {
			goto cleanup;
		}
	}
	else {
		if (qpol_type_get_type_iter(diff->mod_pol->qh, diff->mod_pol->p, attrib, &iter) < 0) {
			goto cleanup;
		}
	}
	for ( ; !qpol_iterator_end(iter); qpol_iterator_next(iter)) {
...
int attrib_deep_diff(poldiff_t *diff, const void *x, const void *y)
{
	...
	size_t i, j;
	int retval = -1, error = 0;
	if (qpol_type_get_name(diff->orig_pol->qh, diff->orig_pol->p, r1, &name) < 0 ||
