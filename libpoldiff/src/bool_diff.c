...
static int bool_name_comp(const void *x, const void *y, void *arg) {
	...
	char *name1, *name2;
	if (qpol_bool_get_name(p->qh, p->p, c1, &name1) < 0 ||
	    qpol_bool_get_name(p->qh, p->p, c2, &name2) < 0) {
		return 0;
	}
...
apol_vector_t *bool_get_items(poldiff_t *diff, apol_policy_t *policy)
{
	...
	int error = 0;
	if (qpol_policy_get_bool_iter(policy->qh, policy->p, &iter) < 0) {
		return NULL;
	}
...
int bool_comp(const void *x, const void *y, poldiff_t *diff)
{
	...
	qpol_bool_t *c2 = (qpol_bool_t *) y;
	char *name1, *name2;
	if (qpol_bool_get_name(diff->orig_pol->qh, diff->orig_pol->p, c1, &name1) < 0 ||
	    qpol_bool_get_name(diff->mod_pol->qh, diff->mod_pol->p, c2, &name2) < 0) {
		return 0;
	}
	return strcmp(name1, name2);
}
...
int bool_new_diff(poldiff_t *diff, poldiff_form_e form, const void *item)
{
	...
	if ((form == POLDIFF_FORM_ADDED &&
	     qpol_bool_get_name(diff->mod_pol->qh, diff->mod_pol->p, c, &name) < 0) ||
	    ((form == POLDIFF_FORM_REMOVED || form == POLDIFF_FORM_MODIFIED) &&
	     qpol_bool_get_name(diff->orig_pol->qh, diff->orig_pol->p, c, &name) < 0)) {
		return -1;
	}
...
int bool_deep_diff(poldiff_t *diff, const void *x, const void *y)
{
	...
	if (qpol_bool_get_name(diff->orig_pol->qh, diff->orig_pol->p, b1, &name) < 0 ||
	    qpol_bool_get_state(diff->orig_pol->qh, diff->orig_pol->p, b1, &s1) < 0 ||
	    qpol_bool_get_state(diff->mod_pol->qh, diff->mod_pol->p, b2, &s2) < 0 ) {
		error = errno;
		goto cleanup;
	}
	if ( s1 != s2 ) {
