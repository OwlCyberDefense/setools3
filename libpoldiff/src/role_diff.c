...
static int role_name_comp(const void *x, const void *y, void *arg) {
	...
	char *name1, *name2;
	if (qpol_role_get_name(p->qh, p->p, r1, &name1) < 0 ||
	    qpol_role_get_name(p->qh, p->p, r2, &name2) < 0) {
		return 0;
	}
...
apol_vector_t *role_get_items(poldiff_t *diff, apol_policy_t *policy)
{
	...
	int error = 0;
	if (qpol_policy_get_role_iter(policy->qh, policy->p, &iter) < 0) {
		return NULL;
	}
...
int role_comp(const void *x, const void *y, poldiff_t *diff)
{
	...
	qpol_role_t *r2 = (qpol_role_t *) y;
	char *name1, *name2;
	if (qpol_role_get_name(diff->orig_pol->qh, diff->orig_pol->p, r1, &name1) < 0 ||
	    qpol_role_get_name(diff->mod_pol->qh, diff->mod_pol->p, r2, &name2) < 0) {
		return 0;
	}
	return strcmp(name1, name2);
}
...
int role_new_diff(poldiff_t *diff, poldiff_form_e form, const void *item)
{
	...
	if ((form == POLDIFF_FORM_ADDED &&
	     qpol_role_get_name(diff->mod_pol->qh, diff->mod_pol->p, r, &name) < 0) ||
	    ((form == POLDIFF_FORM_REMOVED || form == POLDIFF_FORM_MODIFIED) &&
	     qpol_role_get_name(diff->orig_pol->qh, diff->orig_pol->p, r, &name) < 0)) {
		return -1;
	}
...
static apol_vector_t *role_get_types(poldiff_t *diff, qpol_role_t *role, int which)
{
	...
	if (which == POLDIFF_POLICY_ORIG) {
		if (qpol_role_get_type_iter(diff->orig_pol->qh, diff->orig_pol->p, role, &iter) < 0) {
			goto cleanup;
		}
	}
	else {
		if (qpol_role_get_type_iter(diff->mod_pol->qh, diff->mod_pol->p, role, &iter) < 0) {
			goto cleanup;
		}
	}
...
int role_deep_diff(poldiff_t *diff, const void *x, const void *y)
{
	...
	if (qpol_role_get_name(diff->orig_pol->qh, diff->orig_pol->p, r1, &name) < 0 ||
