...
#ifndef POLSEARCH_TYPE_QUERY_HH
#define POLSEARCH_TYPE_QUERY_HH
 ...
 * Query for types.
 */
class polsearch_type_query:public polsearch_query
{
      public:
	/**
	 * Create a query for types.
	 ...
	 */
	polsearch_type_query(polsearch_match_e m = POLSEARCH_MATCH_ALL) throw(std::invalid_argument);
	/**
		 * Copy constructor
		 * @param rhs The query to copy.
	 */
	polsearch_type_query(const polsearch_type_query & rhs);
	//! Destructor.
	~polsearch_type_query();
	...
	/**
		 ...
		 * @return Always returns POLSEARCH_ELEMENT_TYPE.
	 */
	 ...
	 * @param policy The policy from which to get the types.
	 * @return A vector of all types.
	 * @exception std::bad_alloc Out of memory.
	 * @exception std::runtime_error Unable to get the types from the poliy.
	 */
