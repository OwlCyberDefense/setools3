...
#ifndef POLSEARCH_CAT_QUERY_HH
#define POLSEARCH_CAT_QUERY_HH
...
/**
 * Query for MLS categories.
 */
class polsearch_cat_query:public polsearch_query
{
      public:
	/**
	 * Create a query for MLS categories.
	 ...
	 */
	polsearch_cat_query(polsearch_match_e m = POLSEARCH_MATCH_ALL) throw(std::invalid_argument);
	/**
		 * Copy constructor
		 * @param rhs The query to copy.
	 */
	polsearch_cat_query(const polsearch_cat_query & rhs);
	//! Destructor.
	~polsearch_cat_query();
	...
	/**
		 ...
		 * @return Always returns POLSEARCH_ELEMENT_CATEGORY.
	 */
	 ...
	 * @param policy The policy from which to get the categories.
	 * @return A vector of all categories.
	 * @exception std::bad_alloc Out of memory.
	 * @exception std::runtime_error Unable to get the categories from the poliy.
	 */
